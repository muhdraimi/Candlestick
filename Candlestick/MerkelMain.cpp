...
void MerkelMain::printMenu()
{
    ...
    std::cout << "8: Print Additional Candlestick Plot " << std::endl;
...
void MerkelMain::enterkeyword()
{
    std::cout << "Enter Order Type and Product (E.g. bid, ETH/USDT)" << std::endl;
    std::string input;
    std::getline(std::cin, input);
    ...
    if (tokens.size() != 2)
    {
        ...
        std::string ordertype = tokens[0];
        std::string product = tokens[1];

        printCandleStickData(ordertype,product);
    }
...
void MerkelMain::printCandleStickData(std::string ordertype, std::string product){
    


     if(ordertype == "bid"){
        std::vector<OrderBookEntry> gettingordersbid = orderBook.getOrders(OrderBookType::bid, product);
        std::vector<Candlestick> candles = Candlestick::calculateCandlesticks(gettingordersbid);
        Candlestick::printCandlestickTable(candles);
     }
     if(ordertype == "ask"){
        std::vector<OrderBookEntry> gettingorders = orderBook.getOrders(OrderBookType::ask,product);
        std::vector<Candlestick> candles = Candlestick::calculateCandlesticks(gettingorders);
        Candlestick::printCandlestickTable(candles);
     }
